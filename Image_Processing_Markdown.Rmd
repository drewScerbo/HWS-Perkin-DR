...
Once the files are sorted, the other directories that are at the same level are found and ordered. This process uses two functions from the other script; compare_dates and sort_files. 
...
Once the directories are ordered, there is a check to see if there is enough bias frames. A function is run on each nearby directory, in order, to search for bias frames. This process uses the get_files_of_type function which will get all of either the bias frames or the flat fields in a given directory. Bias frames are added to a list and returned. If a flat field is needed, the filters that are needed (same filters as the light frames) are given and check against. 
...
Once enough (at least 3) bias frames are found, they are averaged together to create a master bias frame. Flat fields will continue to run until their are at least 3 fields from each filter needed. This is done by giving a filter vector to the get_files_of_type function and then reevaluating which filter are needed each iteration. 
...
## Calibration Process
Dark subtraction only occurs when mean of scaled dark is less than the median absolute deviation of the master bias. This is to prevent when the scaled dark has negative values that would add instead of subtract noise to the light image.
